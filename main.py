...
class MouseInput:
    ...
    def __send_input(self, input_type, code, value):
        ...
        libc.write(self.handle, pointer(end), sizeof(start))
        libc.write(self.handle, pointer(start), sizeof(end))
...
class InterfaceList:
    def __init__(self):
        console("log", "Starting InterfaceList")
        ...
        console("log", "Started InterfaceList!")
...
class NetVarList:
    ...
    def __init__(self):
        console("log", "Creating NetVarTable")
        ...
        if g_glow:
            console("log", "Importing NetVarTable ESP")
            ...
            console("log", "Imported NetVarTable ESP!")
        console("log", "Created NetVarTable!")
...
def console(prefix, input):
    print("["+prefix+"] "+input)

def rawcommand(input):
    x = json.dumps(input)
    y = json.loads(x)
    return y
        ...
        if Engine.is_in_game():
            try:
                ...
                for i in range(0, mem.read_i32(nv.dwGlowObjectManager + 0x10)):
                   ...
                   mem.write_float(nv.dwGlowPointer + index + 0x0C, entity_health)        # g // If heal low = color low
            ...
            g_previous_tick = 0
            target_set(Player(0))
    console("log", "Not injected... retry.")

sg.theme('Topanga')

layout = [  [sg.Text('Console')],
            [sg.Output(size=(100,25), key='Console', tooltip='Console Output')],
            [sg.In(size=(100, 5), key='Command', tooltip='Console Input')],
            [sg.Button('Execute', tooltip='Execute Python'), sg.Button('Inject', tooltip='Inject Memory'), sg.Button('Exit', tooltip='Exit CS:CK')]  ]

window = sg.Window('CSCK Console v1.0', layout, size=(700, 500))
...
while True:
    ...
    execute = True
    if event in (sg.WIN_CLOSED, 'Inject'):
        inject()
        execute = False
    if event in (sg.WIN_CLOSED, 'Exit'):
        break
        execute = False
    if event in (sg.WIN_CLOSED, 'About'):
        sg.Popup('Ok clicked', keep_on_top=True, size=(700, 500))
        execute = False
    if command == "!inject":
        inject()
        execute = False
